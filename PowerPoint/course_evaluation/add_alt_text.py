        ...
        if shape.shape_type == 13:  # Check if the shape is an image
            ...
            if not adec_decoratives:
                if shape._element._nvXxPr.cNvPr.attrib.get("descr", "") == "":
                    ...
                    response = ppxt_vision.generate_alt_text(image_filename)
                    response = response['choices'][0]['message']['content']
                    
                    shape._element._nvXxPr.cNvPr.attrib["descr"] = response + "\nGenerated Description"  # Set the alt text for the image
